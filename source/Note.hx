...
class Note extends FlxSprite
{
	...
	public function new(_strumTime:Float, _noteData:Int, ?_prevNote:Note, ?sustainNote:Bool = false)
	{
		...
		if(isSustainNote && prevNote.holy) { 
			holy = true;
		}
				...
				if(!holy)
					{
						if(FlxG.save.data.arrowColorCustom)
							frames = Paths.getSparrowAtlas('NOTE_assets_tetris'); //make alt version later
						else
							frames = Paths.getSparrowAtlas('NOTE_assets_tetris');
					...
					{
						loadGraphic(Paths.image('holynote', 'shared'));
					}
