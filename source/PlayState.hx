...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'connect':
				dialogue = CoolUtil.coolTextFile(Paths.txt('connect/connectDialogue'));
		}
		...
		switch(stageCheck)
		{
			...
			case 'stage':
				...
				case 'subway':
				{
						...
						var Train:FlxSprite = new FlxSprite(-500, -100).loadGraphic(Paths.image('mami/BGTrain', 'shared'));
						Train.updateHitbox();
						...
						var stageFront:FlxSprite = new FlxSprite(-500, 600).loadGraphic(Paths.image('mami/BGFloor', 'shared'));
						stageFront.updateHitbox();
						stageFront.antialiasing = true;
						stageFront.scrollFactor.set(0.9, 0.9);
						stageFront.active = false;
						...
						var Lamps:FlxSprite = new FlxSprite(-500, -400).loadGraphic(Paths.image('mami/BGLamps', 'shared'));
						Lamps.updateHitbox();
						Lamps.antialiasing = true;
						Lamps.scrollFactor.set(0.9, 0.9);
						Lamps.active = false;
						add(Lamps);

						var LampsL:FlxSprite = new FlxSprite(-500, -400).loadGraphic(Paths.image('mami/BGLampLights', 'shared'));
						LampsL.updateHitbox();
						LampsL.antialiasing = true;
						LampsL.scrollFactor.set(0.9, 0.9);
						LampsL.active = false;
						add(LampsL);

						var WeebGorl:FlxSprite = new FlxSprite(-530, -20).loadGraphic(Paths.image('mami/BGYes', 'shared'));
						...
						var Assets:FlxSprite = new FlxSprite(-530, -50).loadGraphic(Paths.image('mami/BGRandomshit', 'shared'));
						Assets.updateHitbox();
						Assets.antialiasing = true;
						Assets.scrollFactor.set(0.9, 0.9);
						Assets.active = false;
						add(Assets);				

						gorls = new FlxSprite(-360, 150);
						gorls.frames = Paths.getSparrowAtlas('mami/BGGirlsDanec', 'shared');
